 ...
 * Mirrored verbatim from ../../../design-system/tailwind-theme.ts (the canonical
 * source, generated from design-system/tokens.json). Do not hand-edit values here
 * independently of that file — see design-system/02_DESIGN_TOKENS.md.
 * Consumes the CSS custom properties defined in ../styles/tokens.css so light/dark
 * and the data-theme override keep working without a second source of truth.
 *
 * One addition beyond the design-system source: an `opacity` scale mapping
 * tokens.json's opacity category (disabled/hover-wash/active-wash/scrim), which
 * the upstream tailwind-theme.ts defines in tokens.json but doesn't yet wire to
 * Tailwind. Flagged here so a future regeneration pass folds it back upstream.
 *
 * v1.1 — Tuba Brand Identity Correction: mirrors design-system's corrected Ink
 * (real Tuba purple, #2A0C72) and Coral (renamed from Ember, real Tuba coral,
 * #F95A60) primitives. See design-system/TBOS_DESIGN_SYSTEM_V1_1_BRAND_CORRECTION_REPORT.md.
 */

const theme = {
  colors: {
    // Canonical, exact Tuba brand identity swatches (not a tint/shade scale) — see design-system/03_COLOR_SYSTEM.md §1a.
    brand: {
    ...
    chart: {
      1: 'var(--chart-1-blue)',
      2: 'var(--chart-2-orange)',
      3: 'var(--chart-3-violet)',
      4: 'var(--chart-4-yellow)',
      5: 'var(--chart-5-magenta)',
      6: 'var(--chart-6-green)',
      7: 'var(--chart-7-aqua)',
      8: 'var(--chart-8-red)',
      'status-good': 'var(--chart-status-good)',
      'status-warning': 'var(--chart-status-warning)',
      'status-serious': 'var(--chart-status-serious)',
      'status-critical': 'var(--chart-status-critical)',
    },
  },
  ...
  /** grid.wide.contentMaxWidth + sizing.panelWidth from tokens.json — wired here
   * so layout/overlay components never need an arbitrary-value bracket class. */
  maxWidth: { content: '1440px', 'panel-sm': '25rem', 'panel-md': '35rem', 'panel-lg': '45rem' },
